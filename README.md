<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:7C3AED,100:3B82F6&height=280&section=header&text=Danish%20Raza&fontSize=50&fontColor=ffffff&fontAlignY=35&animation=fadeIn&desc=MERN%20Stack%20Developer%20%7C%20AI%20Integration%20Specialist&descAlignY=60&descSize=16" width="100%"/>

<a href="https://danishraza.dev">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&duration=3000&pause=1000&color=7C3AED&center=true&vCenter=true&width=560&lines=Building+AI-powered+products+end-to-end;RAG+Pipelines+%C2%B7+MERN+%C2%B7+Gemini+API;World+Cup+2026+%E2%9A%BD+meets+AI"/>
</a>

<br/>

<a href="https://danishraza.dev"><img src="https://img.shields.io/badge/danishraza.dev-7C3AED?style=flat-square&logo=safari&logoColor=white"/></a>
<a href="https://github.com/danishrazabangash"><img src="https://img.shields.io/badge/danishrazabangash-161B22?style=flat-square&logo=github&logoColor=white"/></a>
<a href="https://www.linkedin.com/in/danish-raza-bangash/"><img src="https://img.shields.io/badge/Connect-2563EB?style=flat-square&logo=linkedin&logoColor=white"/></a>
<a href="mailto:razadanish330@gmail.com"><img src="https://img.shields.io/badge/Reach_Out-6D28D9?style=flat-square&logo=gmail&logoColor=white"/></a>

</div>

<br/>

## 🧭 About

```yaml
role:       MERN Stack Developer & AI Integration Specialist
education:  BS Computer Science, University of Peshawar (GPA 3.9, Class of 2026)
location:   Islamabad / Peshawar, Pakistan — open to remote
focus:      Full-stack AI products — RAG pipelines, chat systems, real deployment
looking_for: First professional role at a product-focused startup or AI team
```

<br/>

## ✨ Featured Work

<table>
<tr>
<td width="50%" valign="top">

### 🤖 [BotForge](https://chatbot-builder-api-2v3u.onrender.com/)
**No-code AI chatbot builder** — my Final Year Project, fully Dockerized and in production.

- Four-tier parallel RAG pipeline (semantic · full-text · regex · fuzzy)
- `gemini-embedding-2` → **~90% retrieval accuracy** at **~780ms** latency
- WhatsApp Cloud API + Telegram integrations, multi-tenant webhooks
- Four-tier subscription model, self-hostable by design

`MongoDB` `Express` `React` `Node.js` `Docker` `Gemini API`

</td>
<td width="50%" valign="top">

### ⚽ [Footalyzer](https://footalyzer.app)
**World Cup 2026 AI companion app** — making football understandable through AI.

- AI-generated match briefings + conversational chatbot
- Real-time data via API-Football
- Background jobs with BullMQ + Redis, live updates via Socket.io

`Next.js 14` `Express` `MongoDB` `Redis` `Clerk` `Stripe`

</td>
</tr>
</table>

<br/>

## 🛠️ Tech Stack

<div align="center">

**Core**
<br/>
<img src="https://skillicons.dev/icons?i=js,react,nextjs,nodejs,express,tailwind&theme=dark" />

**Data & Infra**
<br/>
<img src="https://skillicons.dev/icons?i=mongodb,redis,docker,git,github,vscode&theme=dark" />

**AI**
<br/>
<img src="https://img.shields.io/badge/Google_Gemini-7C3AED?style=flat-square&logo=googlegemini&logoColor=white"/>
<img src="https://img.shields.io/badge/RAG_Pipelines-3B82F6?style=flat-square"/>
<img src="https://img.shields.io/badge/BullMQ-EF4444?style=flat-square"/>
<img src="https://img.shields.io/badge/WhatsApp_Cloud_API-25D366?style=flat-square&logo=whatsapp&logoColor=white"/>

</div>

<br/>

## 📊 GitHub Stats

<div align="center">
<img src="https://github-stats-extended.vercel.app/api?username=danishrazabangash&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&bg_color=0D1117" width="48%" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=danishrazabangash&theme=tokyonight&hide_border=true&background=0D1117" width="48%" />
</div>

<div align="center">
<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=danishrazabangash&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117" width="60%" />
</div>

<br/>

<div align="center">
<i>Open to MERN Stack Developer &amp; AI Integration roles — let's build something great.</i>
</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:3B82F6,100:7C3AED&height=120&section=footer" width="100%"/>
